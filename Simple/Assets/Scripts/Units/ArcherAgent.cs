...
public class ArcherAgent : MonoBehaviour, IDamageable
{
    ...
    public bool HasTaskAssigned { get; set; } = false;
    ...
    void Start()
    {
        ...
        {
            Debug.Log("NavMeshAgent component not found on the archer, disabling script.");
            this.enabled = false;
        }
    ...
    void Update()
    {
        ...
        if (currentState == State.Moving && !navMeshAgent.pathPending && navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)
        {
            if (!navMeshAgent.hasPath || navMeshAgent.velocity.sqrMagnitude == 0f)
            {
                ...
                Debug.Log("Unit has reached its destination and is now idle.");
            }
        }
    }
    ...
    private void Die()
    {
        ...
        StopAllCoroutines();  // This will ensure all coroutines are stopped when the archer dies
        ...
        UnitMLManager.Instance.RemoveArcher(gameObject);
    ...
    private void CheckForEnemiesInAggroRadius()
    {
        ...
        {
            if (col.gameObject.CompareTag("EnemyPrefabs"))
            {
    ...
    {
        bool isAttacking = true; // Flag to indicate if the archer is currently attacking
